...
function setMap() {
    ...
    Promise.all(promises).then(function(data) {  // Wait for all promises to resolve
        ...
        // // Main parish rendering and interaction logic
        // g.selectAll(".parish")                       // Select all existing parish elements (empty selection on first run)
        // .data(allParishes.features)                  // Bind parish GeoJSON features to selection
        // .enter()                                     // Get enter selection for new data elements
        // .append("path")                              // Create SVG path element for each parish
        // .attr("class", d => `parish ${                // Set CSS classes:
        //     d.properties.isCancerAlley ? 'cancer-alley' : ''  // Add 'cancer-alley' class if applicable
        // }`)
        // .attr("d", path)                             // Generate path data using geographic projection
        // .style("fill", d =>                          // Set fill color:
        //     d.properties.home_price ?                // If home price exists:
        //         colorScale(d.properties.home_price)  // Use color scale value
        //         : "#f5f5f5"                         // Default light gray for missing data
        // )
        // .style("stroke", d =>                        // Set border color:
        //     d.properties.isCancerAlley ? "#ff0000"   // Red for Cancer Alley
        //     : "#fff"                                // White for others
        // )
        // .style("stroke-width", d =>                  // Set border width:
        //     d.properties.isCancerAlley ? "2px"       // Thicker for Cancer Alley
        //     : "0.5px"                               // Thin for others
        // )

        // // Mouseover interaction handler
        // .on("mouseover", function(event, d) {
        //     if (d.properties.isCancerAlley) {        // Only trigger for Cancer Alley parishes
        //         const hoveredPrice = d.properties.home_price;  // Get reference price
        //         const range = [                      // Calculate price comparison range:
        //             hoveredPrice * 0.9,              // 10% below
        //             hoveredPrice * 1.1               // 10% above
        //         ];
                
        //         // Keep current parish and its label fully visible
        //         d3.select(this).classed("faded", false);
        //         g.selectAll(`.parish-label[data-name="${d.properties.NAME}"]`)
        //             .classed("faded", false);
                
        //         // Highlight similarly-priced non-Cancer Alley parishes
        //         g.selectAll(".parish:not(.cancer-alley), .parish-label:not(.cancer-alley-label)")
        //             .classed("highlighted", p =>     // Apply highlight if:
        //                 p.properties?.home_price >= range[0] &&  // Price within range
        //                 p.properties?.home_price <= range[1]
        //             )
        //             .classed("faded", false);        // Ensure they're not faded
                
        //         // Fade all other elements except:
        //         // - Current parish
        //         // - Similarly-priced non-Cancer Alley parishes
        //         g.selectAll(".parish, .parish-label, .mississippi")
        //             .classed("faded", p => 
        //                 (p.properties &&             // Has properties
        //                 p !== d &&                   // Not the current parish
        //                 !(                          // Not both:
        //                     p.properties.home_price >= range[0] &&  // In price range
        //                     p.properties.home_price <= range[1] &&
        //                     !p.properties.isCancerAlley              // And not Cancer Alley
        //                 ))
        //             );
        ...
        // // Create and position the color legend
        // setLegend(colorScale, homePriceData);            // Pass color scale and data reference
...
function callback(data) {
    ...
    // // Create quantitative color scale for home prices
    // const colorScale = makeColorScale(homePriceData);  // Generates a color gradient function

    // // Draw all parish boundaries and set up interactions
    // g.selectAll(".parish")  // Select parish elements (empty selection initially)
    //     .data(allParishes.features)  // Bind parish data
    //     .enter()  // Get enter selection for new data
    //     .append("path")  // Create SVG path for each parish
    //     .attr("class", d => `parish ${d.properties.isCancerAlley ? 'cancer-alley' : ''}`)  // Conditional class
    //     .attr("d", path)  // Generate path using projection
    //     .style("fill", d => colorScale(d.properties.home_price))  // Color by home price
    //     .style("stroke", "#fff")  // Default white border
    //     .style("stroke-width", "0.5px")  // Thin border
    //     .on("mouseover", function(event, d) {
    //         // Only trigger interactions for Cancer Alley parishes
    //         if (d.properties.isCancerAlley) {
    //             const hoveredPrice = d.properties.home_price;  // Get reference price
    //             const range = [hoveredPrice * 0.9, hoveredPrice * 1.1];  // ±10% price range
                
    //             // Keep hovered Cancer Alley parish visible
    //             d3.select(this).classed("faded", false);
    ...
    //                     p.properties.home_price <= range[1]
    //                 )
    //                 .classed("faded", false);  // Ensure they're visible
                
    //             // Fade other elements except:
    //             // - Current parish
    //             // - Highlighted matches
    //             g.selectAll(".parish, .mississippi, .tri-site")
    //                 .classed("faded", p => {
    //                     // For parishes (check properties exist)
    //                     if (p.properties) {
    //                         return p !== d &&  // Not current parish
    //                                !(!p.properties.isCancerAlley &&  // Not a highlighted match
    //                                  p.properties.home_price >= range[0] && 
    //                                  p.properties.home_price <= range[1]);
    //                     }
    //                     // For non-parish elements (river, TRI sites) - always fade
    //                     return true;
    //                 });
    //         }
    //     })
    //     .on("mouseout", function() {
    //         // Reset all visual states on mouseout
    //         g.selectAll(".parish, .mississippi, .tri-site")
    //             .classed("highlighted", false)  // Remove highlights
    //             .classed("faded", false);  // Remove fades
    //     });

    // // Apply special styling to Cancer Alley parishes
    // g.selectAll(".cancer-alley")  // Select just Cancer Alley parishes
    //     .style("fill", "url(#caHatch)")  // Apply pattern fill (defined elsewhere)
    //     .style("stroke", "red")  // Red border
    //     .style("stroke-width", "2px");  // Thicker border
}
